...
document.getElementById("button1").onchange = function () {
   ...
   let file = this.files[0]; // Corrected: changed .file to .files
...
function generateWordFreq(inputFile, outputDoc, outputCount) {
   ...
   let fr = new FileReader(); // Corrected: changed Reader to FileReader
   fr.readAsText(inputFile); // Corrected: changed read to readAsText
   ...
   fr.onload = function () {
      ...
      let sourceText = outputDoc.textContent; // Using textContent is safer than innerHTML

      // Remove any character that is not alphabetic or whitespace
      let alphaRegx = /[^a-zA-Z\s]/g; // Corrected: Removed quotes from regex literal
      sourceText = sourceText.replace(alphaRegx, "").toLowerCase(); // Convert to lower case for consistency
      ...
      for (let i = 0; i < words.length; i++) { // Corrected: changed <= to <
      ...
      for (let i = 1; i <= 15; i++) {
         ...
         let percent = (freqs[i] / totalWords * 100).toFixed(1) + "%"; // Corrected: Inverted calculation
