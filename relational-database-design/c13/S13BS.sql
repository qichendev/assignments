...
SELECT
  ...
  COALESCE(TO_CHAR(m.employee_id), '---') AS "MGR#"
FROM b_employees e
...
JOIN b_employees m
  ON e.manager_id = m.employee_id
WHERE m.employee_id IN (110, 111, 112)
ORDER BY m.employee_id, e.employee_id;

-- 4 --
SELECT
  e.first_name || ' ' || e.last_name AS "EMPLOYEE",
  TO_CHAR(e.hire_date, 'Mon DD, YYYY') AS "EMP_HIRE_DATE",
  m.first_name || ' ' || m.last_name AS "MANAGER",
  TO_CHAR(m.hire_date, 'Mon DD, YYYY') AS "MGR_HIRE_DAT"
FROM b_employees e
JOIN b_employees m
  ON e.manager_id = m.employee_id
...
SELECT
  ...
  TO_CHAR(SUM(ol.price_paid * ol.quantity), '$9,999.99') AS "ORDER_TOTAL"
FROM b_orders o
JOIN b_customers c
