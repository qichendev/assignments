...
SELECT 
  ...
  UPPER(street || ' ' || cust_city || ', ' || cust_state || ' ' || cust_zip) AS "Address"
FROM 
  b_customers
WHERE 
  cust_state IN ('WI', 'SC', 'NY')
ORDER BY 
  customer_name;

SELECT 
  LOWER(last_name || ', ' || first_name) AS "Employee Name",
  LOWER(street || ' ' || emp_city || ', ' || emp_state || ' ' || emp_zip) AS "Address"
FROM 
  b_employees
WHERE 
  employee_id BETWEEN 104 AND 109
ORDER BY 
  "Employee Name";

SELECT 
  'Customer id ' || customer_id AS "Customer id",
  UPPER(customer_name) AS "Customer Name"
FROM 
  b_customers
WHERE 
  LOWER(cust_city) = 'san diego'
ORDER BY 
  "Customer Name";

SELECT 
  UPPER(cust_city) AS "City",
  LENGTH(TRIM(cust_city)) AS "City Length"
FROM 
  b_customers
GROUP BY 
  UPPER(cust_city), cust_city
ORDER BY 
  "City";
...
FROM 
  b_customers
WHERE 
  UPPER(SUBSTR(cust_city, 1, 1)) = 'S'
ORDER BY 
  customer_name;

SELECT LENGTH('Database Design and SQL.') AS "Length" FROM DUAL;

SELECT 'Database' || ' ' || 'Design' || ' ' || 'and' || ' ' || 'SQL' AS "TITLE" FROM DUAL;

SELECT LPAD('Database', 12, '*') || LPAD('Internet', 12, '*') || LPAD('Academy', 12, '*') AS "DIA" FROM DUAL;

SELECT RPAD('Database', 11, '$') || RPAD('Internet', 11, '$') || 'Academy' AS "DIA" FROM DUAL;
...
SELECT 
  ...
  '$' || TO_CHAR(price, '9999.99') AS "PRICE"
FROM 
  b_products
WHERE 
  price > 1500.00
...
FROM 
  b_products
WHERE 
  UPPER(category_code) = UPPER(:Enter_category_code)
