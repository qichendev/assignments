...
-- SELECT STATEMENTS
-- 1
SELECT TO_CHAR(AVG(price), '$9999.99') AS "AVERAGE_PRICE"
FROM b_prices
WHERE end_date IS NOT NULL;

-- 2
SELECT TO_CHAR(SUM(price_paid), '$9,999.99') AS "TOTAL_ORDERS"
FROM b_order_lines;

-- 3
SELECT TO_CHAR(MIN(monthly_salary), '$9,999.99') AS "LOWEST_SALARY",
    TO_CHAR(MAX(monthly_salary), '$9,999.99') AS "HIGHEST_SALARY",
    TO_CHAR(AVG(monthly_salary), '$9,999.99') AS "AVERAGE_SALARY",
...
-- 5
SELECT COUNT(product_code) AS "NUMBER_OF_PRODUCTS",
    TO_CHAR(AVG(price), '$9999.99') AS "AVERAGE_PRICE"
FROM b_products
...
FROM b_orders
WHERE order_date BETWEEN DATE '2020-01-01' AND DATE '2020-12-31';

-- 7
SELECT COUNT(customer_id) AS "COUNT",
    TO_CHAR(AVG(credit_limit), '$9,999.99') AS "AVG_CR_LIMIT",
    TO_CHAR(AVG(balance), '$9,999.99') AS "AVG_BALANCE",
    TO_CHAR(SUM(credit_limit - balance), '$9,999.99') AS "TOTAL_BLANCE",
    TO_CHAR(AVG(credit_limit - balance), '$9,999.99') AS "Avg Cr Avail"
FROM b_customers;
...
-- 9
SELECT COUNT(customer_id) AS "Customer with no discount"
FROM b_customers
...
SELECT cust_state AS "State",
    ...
    CASE 
        WHEN discount IS NULL THEN '0.005'
        ELSE TO_CHAR(discount)
    END AS "Discount"
FROM b_customers
WHERE cust_state IN ('CO', 'NY')
...
SELECT cust_state AS "State",
    customer_name AS "Customer Name",
    CASE 
        WHEN discount IS NULL THEN 'Not Assigned'
        ELSE TO_CHAR(discount)
    END AS "Discount"
FROM b_customers
...
SELECT order_date AS "Order Date",
    ...
    CASE 
        ...
        ELSE TO_CHAR(ship_date)
    END AS "Ship Date"
FROM b_orders
ORDER BY order_date, order_id;

-- 1
SELECT category_code AS "Category Code",
    TO_CHAR(AVG(price), '$9,999.99') AS "Average Price"
FROM b_products
WHERE price >= 1500
...
SELECT EXTRACT(YEAR FROM membership_date) AS "Year",
    ...
    COUNT(customer_id) AS "Number of Customers"
FROM b_customers
GROUP BY EXTRACT(YEAR FROM membership_date), EXTRACT(MONTH FROM membership_date)
ORDER BY EXTRACT(YEAR FROM membership_date), EXTRACT(MONTH FROM membership_date);
